...
- 指示ファイルを読み込めない場合は警告を出してそのファイルだけをスキップし、チャットのターン全体は継続します。
...
一致した指示ファイルを読み込めない場合、OpenCodeプロセスのログへ警告を出し、そのファイルを適用せずにターンを継続します。重要な指示ファイルは、常に読み取れる安定した場所へ保存してください。
